...
/* Read constants and initial conditions from br2dtask.dat */
{
  ...
  if((fp=fopen("brdr2dtask.dat","r"))==NULL) {
    ...
    for (i=0;i<Nx+2;++i){  
      for (j=0;j<Ny+2;++j){
        for (k=0;k<varnum;++k){ 
	  for (l=0;l<datarr4dim;++l) datarr[i][j][k][l]=0.0;
	}
      }
    }
    ...
    fscanf(fp,"%lf",&datarr[1][1][0][0]); next_line(fp);    /* Vm (mV) */
    fscanf(fp,"%lf",&datarr[1][1][11][0]); next_line(fp);   /* Cai   (mole/L) */
    ...
    datarr[1][1][4][0]=abfun(datarr[1][1][0][0],0)/(abfun(datarr[1][1][0][0],0)+abfun(datarr[1][1][0][0],1));      // x1 initial condition
    datarr[1][1][6][0]=abfun(datarr[1][1][0][0],2)/(abfun(datarr[1][1][0][0],2)+abfun(datarr[1][1][0][0],3));      // m initial condition
    datarr[1][1][7][0]=abfun(datarr[1][1][0][0],4)/(abfun(datarr[1][1][0][0],4)+abfun(datarr[1][1][0][0],5));      // h initial condition
    datarr[1][1][9][0]=abfun(datarr[1][1][0][0],8)/(abfun(datarr[1][1][0][0],8)+abfun(datarr[1][1][0][0],9));     // d initial condition
    datarr[1][1][10][0]=abfun(datarr[1][1][0][0],10)/(abfun(datarr[1][1][0][0],10)+abfun(datarr[1][1][0][0],11));  // f initial condition

    // Propagate initial conditions to all other nodes
    for (i=1;i<Nx+1;++i){
      for (j=1;j<Ny+1;++j){
        if (i>1 | j>1) { for (k=0;k<varnum;++k) datarr[i][j][k][0]=datarr[1][1][k][0]; }
      }
    }
    ...
    if (ictype!=2) {
      printf("\nInitial Conditions: \n"); 
      printf("        Vm (mV): %3.3f\n",datarr[middlex][middley][0][0]);
      printf("  IK1 (uA/cm^2): %3.3f\n",datarr[middlex][middley][2][0]);
      printf("  Ix1 (uA/cm^2): %3.3f\n",datarr[middlex][middley][3][0]);
      printf("  x1 (unitless): %3.3f\n",datarr[middlex][middley][4][0]);
      printf("  INa (uA/cm^2): %3.3f\n",datarr[middlex][middley][5][0]);
      printf("   m (unitless): %3.3f\n",datarr[middlex][middley][6][0]);
      printf("   h (unitless): %3.3f\n",datarr[middlex][middley][7][0]);
      printf("   Is (uA/cm^2): %3.3f\n",datarr[middlex][middley][8][0]);
      printf("   d (unitless): %3.3f\n",datarr[middlex][middley][9][0]);
      printf("   f (unitless): %3.3f\n",datarr[middlex][middley][10][0]);
      printf("   Cai (mole/L): %3.3e\n",datarr[middlex][middley][11][0]);  
      printf("\n");}
  ...
  for (i=1;i<Nx+1;i=i+wdN){
    for (j=1;j<Ny+1;j=j+wdN){
      for (k=0;k<outfilenum-1;++k) {                                                   // save the data
        if (k==13 && outfilef[13]==1) fwrite(&datarr[i][j][k][(step-1)%2], sizeof(double),1,outfile[k]);  // save diffusion
        else if (outfilef[k]==1) fwrite(&datarr[i][j][k][step%2], sizeof(double),1,outfile[k]);            // save others
      }
    }
  }
  // Update info file
...
// datarr: (rows)X(columns)X(depth)X(4dim)=(Nx)X(Ny)X(varnum)X(datarr4dim)
{
  ...
  for (k=0;k<varnum;++k) { 
    if (k==13) {                     // Save the next diffusion matrix: [(step-1)%2]
      for (i=1;i<Nx+1;++i){
        for (j=1;j<Ny+1;++j) fwrite(&datarr[i][j][k][(step-1)%2],sizeof(double),1,fcfile);  // save the data
      }
    }
    else {                        // Save current data for all other variables: [(step)%2]
      for (i=1;i<Nx+1;++i){
        for (j=1;j<Ny+1;++j) fwrite(&datarr[i][j][k][(step)%2],sizeof(double),1,fcfile);  // save the data
      }
    }  
  }
}
