...
//	dCaidt = derivarr+index+6;   /* mole/L */
    ...
    *(derivarr + index + 1) = dx1dt;
    *(derivarr + index + 2) = dmdt;
    *(derivarr + index + 3) = dhdt;
    *(derivarr + index + 4) = dddt;
    *(derivarr + index + 5) = dfdt;
    *(derivarr + index + 6) = dCaidt;
...
//    Isum = *(d_datarr + index + 12*2);    /* uA/cm^2 */
    ...
    *(d_datarr + index + d3(2)) = IK1;	 

    /* Ix1 */
    x1 = x1 + dx1dt*dt;
    *(d_datarr + index + d3(4)) = x1;
    Ix1t1 = exp(0.04*(Vm+77)) - 1;
    Ix1t2 = exp(0.04*(Vm+35));
    Ix1 = ((gx1*Ix1t1) / (Ix1t2)) * (x1);        /* uA/cm^2 */
    *(d_datarr + index + d3(3)) = Ix1;
	 

    /* INa */
    m = m + dmdt * dt;
    *(d_datarr + index + d3(6)) = m;
    h = h + dhdt * dt;
    *(d_datarr + index + d3(7)) = h;
    ...
    INa = ((gNa) * (*(block+X*(Ny+2) +Y))
			 * (pow((m),3.0))*(h))*(Vm-ENa);         
    *(d_datarr + index + d3(5)) = INa;	 

    /* Cai */
    Cai = Cai + dCaidt * dt;              /* moles/L */
    *(d_datarr + index + d3(11))=Cai;
	 

    /* Is */
    d = d + dddt * dt;
    *(d_datarr + index + d3(9))=d; 
	 
    f = f + dfdt * dt;
    *(d_datarr + index + d3(10))=f; 
    ...
    *(d_datarr+index) = Vm + ((dVmdt)*dt);                          /* mV */
    ...
    /* Diffusion: */
    /* Compute the new Vm's contribution to the next diffusion matrix */
    /* (step-1)%2 for next timestep and step%2 for current timestep */
    /* Dp stands for Dprime:*/
    /* the Laplacian multiplier form of the diffusion tensor D */
    /* Remember: diffusion at ghost points doesn't matter  */
    /* Push model --> Pull model */ 
     
    /* depend on bcs function, executed after bcs, see separate kernel */
/*    
    index = d4(step%2) + d3(13)+d2(X)+Y;
    *(d_datarr + index) = 0.0; */  // comment to match OpenCL implementation
}
